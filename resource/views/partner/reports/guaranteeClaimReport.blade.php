    ...
    <div class="content-header row">
        <div class="content-header-left col-md-9 col-12 mb-2">
            <div class="row breadcrumbs-top">
                <div class="col-12">
                    <h2 class="content-header-title float-left mb-0">{{ __('messages.guarantee_claim_report') }}
                    </h2>
                    <div class="breadcrumb-wrapper">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="#">{{ __('messages.report') }}</a>
                            </li>
                            <li class="breadcrumb-item active">{{ __('messages.guarantee_claim_report') }}
                            </li>
                        </ol>
                    </div>
                </div>
        ...
        <section id="team">
            ...
            <div class="row" id="table-responsive">
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            <h4 class="card-title">{{ __('messages.guarantee_claim_report') }}</h4>
                        </div>
                        <div class="card-body">
                            <h6 class="card-subtitle">{{ __('messages.date_from') }} {{$date_from}} {{__('messages.to')}} {{$date_to}} | {{__('messages.total_request_amount')}} : {{$total_request_amount}} |{{__('messages.total_final_verify_amount')}} : {{$total_final_verify_amount}}</h6>
                        </div>
                            ...
                            <table class="table table-hover mb-0">
                                <thead>
                                    <tr>
                                        ...
                                        <th scope="col" class="text-nowrap">{{ __('messages.third_party_username') }}
                                        </th>
                                        <th scope="col" class="text-nowrap">{{ __('messages.request_amount') }}</th>
                                        <th scope="col" class="text-nowrap">{{ __('messages.final_verify_amount') }}
                                        </th>
                                    ...
                                    @if (count($report))
                                        @foreach ($report as $param)
                                            <tr>
                                                ...
                                                <td>{{ $param->final_verify_amount }}</td>
                                                <td>
                                                    @if($param->state == 1)
                                                        Approved
