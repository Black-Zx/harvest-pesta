    ...
    <div class="content-header row">
        <div class="content-header-left col-md-9 col-12 mb-2">
            <div class="row breadcrumbs-top">
                <div class="col-12">
                    <h2 class="content-header-title float-left mb-0">{{ __('messages.insurance_report') }}
                    </h2>
                    <div class="breadcrumb-wrapper">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="#">{{ __('messages.report') }}</a>
                            </li>
                            <li class="breadcrumb-item active">{{ __('messages.insurance_report') }}
                            </li>
                        </ol>
                    </div>
                </div>
        ...
        <section id="team">
                        ...
                        <div class="card-body">
                            <form method="GET">
                                ...
                                <div class="col-md-6 col-12">
                                    ...
                                    <div class="col-md-6 col-12">
                                        <div class="form-group">
                                            <label for="is_test">{{ __('messages.account_type') }}
                                            </label>
                                            <select class="form-control" id="is_test" name="is_test" required>
                                                <option value="2" {{ app('request')->input('is_test') == 2 ? 'selected' : '' }}>{{ __('messages.all_account') }}</option>
                                                <option value="0" {{ app('request')->input('is_test') == 0 ? 'selected' : '' }}>{{ __('messages.non_testing_account') }}</option>
                                                <option value="1" {{ app('request')->input('is_test') == 1 ? 'selected' : '' }}>{{ __('messages.testing_account') }}</option>
                                            </select>
                                        </div>
                                    </div>
                                </div>
            ...
            <div class="row" id="table-responsive">
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            <h4 class="card-title">{{ __('messages.insurance_report') }}</h4>
                        </div>
                        <div class="card-body">
                            <h6 class="card-subtitle">{{ __('messages.date_from') }} {{$date_from}} {{__('messages.to')}} {{$date_to}} | {{__('messages.total_point')}} : {{$total_point}} |{{__('messages.total_point_in')}} : {{$total_point_in}} | {{__('messages.total_point_out')}} : {{$total_point_out}}</h6>
                        </div>
                            ...
                            <table class="table table-hover mb-0">
                                <thead>
                                    <tr>
                                        ...
                                        <th scope="col" class="text-nowrap">{{ __('messages.total_point') }}</th>
                                    ...
                                    @if (count($report))
                                        @foreach ($report as $param)
                                            <tr>
                                                ...
                                                <td>
                                                    <form target="_blank" action="{{ route('cs.reports.insurancePointReportById', ['id' => $param->id]) }}" method="GET"
                                                        class="mb-1">
